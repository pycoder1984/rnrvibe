...
- Image tools (Image Generator, Image Studio, Logo Generator, ControlNet) check Stable Diffusion connectivity on mount and show an error banner with retry if down. Follow this pattern for any new image tools. Tools that depend on a specific A1111 extension (e.g. ControlNet on `sd-webui-controlnet`, or the rembg tab on `stable-diffusion-webui-rembg`) additionally verify the extension itself is loaded — not just that SD is responsive — so the user gets a clear "extension not available" message instead of a generic 502 at submit time. `GET /api/controlnet` is the reference pattern: it proxies the extension's `/controlnet/model_list` and returns `models: []` + an error on failure.
...
- Adding a new tool always requires: a page in `app/tools/<slug>/page.tsx` and a registry entry in `data/tools.ts`. Additionally, **chat-backed tools** (those that POST to `/api/chat` with a `tool` id) need an entry in `ALLOWED_SYSTEM_PROMPTS` in `lib/guardrails.ts` — that's how the server maps the client-supplied id to a hardcoded system prompt so the client can't inject its own. **Direct-SD tools** (e.g. image-generator, controlnet) get their own `app/api/<slug>/route.ts` that calls the A1111 API directly and do NOT use `ALLOWED_SYSTEM_PROMPTS`. Optionally add `app/tools/<slug>/layout.tsx` with per-tool `metadata` + a `FAQPage` JSON-LD block (see `app/tools/chat/layout.tsx` or `app/tools/controlnet/layout.tsx` for the pattern) — the `/faq` page and sitemap pick these up for SEO. Project (demo) metadata lives separately in `lib/project-metadata.ts`.
...
**Known venv gotcha:** the `sd-webui-controlnet` extension depends on `controlnet_aux`, which imports `mediapipe.solutions` at module load. `mediapipe >= 0.10.14` removed that attribute, so `controlnet_aux` raises `AttributeError: module 'mediapipe' has no attribute 'solutions'` and the extension's main alwayson script fails to register — `/controlnet/model_list` still works (so the extension *looks* loaded), but `txt2img` rejects requests with `Script 'controlnet' not found` and the tool surfaces "Connection failed". Diagnose by hitting `GET /sdapi/v1/scripts` and confirming `"controlnet"` is missing from `txt2img` (only `"controlnet m2m"` registers on partial load). **Fix:** `controlnet_aux` 0.0.10 is the latest on PyPI and is *still* broken, and pinning `mediapipe<0.10.14` is also a dead end because it pulls protobuf/numpy versions that conflict with gradio/blendmodes/open-clip-torch in the SD venv. The working fix is to patch `venv\lib\site-packages\controlnet_aux\mediapipe_face\mediapipe_face_common.py` line 15: change `if mp:` to `if mp and hasattr(mp, "solutions"):`. Only the face preprocessor uses those constants, and we don't use face mode (only pose/depth/canny). Restart SD afterward.
